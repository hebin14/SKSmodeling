...
print(".......")
print('script for creating a topo graphy file with boundayies taper to zero for FKSEM')
...
for iz in range(count):
  if(np.abs(lonx[iz]-lonx[0])<0.0001):
    ny+=1
nx=0
for iz in range(count):
  if(np.abs(laty[iz]-laty[0])<0.0001):
    nx+=1
print(nx,ny,count-nx*ny)
topo2d=np.zeros(shape=[nx,ny],dtype=float)
for iy in range(ny):
  for ix in range(nx):
    iyx= iy*nx + ix
    topo2d[ix,iy]=topo[iyx]
