...
integer,PARAMETER   :: UTM_PROJECTION_ZONE = 15
...
program main 
  ...
  ! filename
  character(len=MAX_STR_LEN)  :: COOR_PATH,MODEL_PATH,BACKGROUND_MODEL_PATH,INTFILE,parname,out_name,filename
  
  ! useful arrays
  real(kind=CUSTOM_REAL),ALLOCATABLE   :: vp_read(:,:,:,:),background_vp(:,:,:,:)  ! parameter
  ...
  if(rank == 0) then 
    if(command_argument_count() /= 6) then 
      ...
      call get_command_argument(3,BACKGROUND_MODEL_PATH)
      COOR_PATH = trim(COOR_PATH)
      MODEL_PATH = trim(MODEL_PATH)
      BACKGROUND_MODEL_PATH = trim(BACKGROUND_MODEL_PATH)
      write(*,'(a,a)') 'COOR_PATH = ',adjustl(trim(COOR_PATH))
      write(*,'(a,a)') 'MODEL_PATH = ',adjustl(trim(MODEL_PATH))
      write(*,'(a,a)') 'BACKGROUND_MODEL_PATH = ',adjustl(trim(BACKGROUND_MODEL_PATH))

      call get_command_argument(4,INTFILE)
      INTFILE = trim(INTFILE)
      call get_command_argument(5,parname)
      call get_command_argument(6,out_name)      
    endif
  endif 
  ...
  call MPI_Bcast(BACKGROUND_MODEL_PATH,MAX_STR_LEN,MPI_CHARACTER,0,MPI_COMM_WORLD)
  ...
  ! allocate space
  ALLOCATE(vp_read(NGLLX,NGLLY,NGLLZ,nspec),background_vp(NGLLX,NGLLY,NGLLZ,nspec),ibool(NGLLX,NGLLY,NGLLZ,nspec),stat=ierr)
  ...
  !read vp0
  write(filename,'(a,a,i6.6,a,a,a)') trim(BACKGROUND_MODEL_PATH),'/proc',rank,'_',trim(parname),'.bin'
  open(IIN,file=trim(filename),form='unformatted',iostat=ierr)
  if(ierr /=0 ) stop 'error readind vpfile '
  read(IIN) background_vp
  close(IIN)
  if(rank == 0)  then 
    write(filename,'(a,a,a,a,a)') trim(BACKGROUND_MODEL_PATH),'/proc','_',trim(parname),'.bin'
    print*,'finished reading background parameters '//trim(filename)
  endif
  !read vp
        ...
        do i=1,NGLLX
          ...
          do ipt=1,npts 
            if (dist(ipt) < distmin(ipt)) then 
              vpmin(ipt) = (vp_read(i,j,k,ispec)-background_vp(i,j,k,ispec))/background_vp(i,j,k,ispec)*100
  ...
  ! deallocate space
  DEALLOCATE(xstore,ystore,zstore,ibool,vp_read,background_vp)
