  ...
  do imat = 1, nmaterials
    ...
    if (ANISOTROPY .and. IMODEL == IMODEL_TOMO) then
      ...
      if (myrank_tomo == 0) then
        ...
        write(IMAIN,*) ' max/min Gc :' ,maxval((c_tomography(:,:,16)-c_tomography(:,:,19))&
                                              /(c_tomography(:,:,16)+c_tomography(:,:,19)+1.0e-20)),&
                                        minval((c_tomography(:,:,16)-c_tomography(:,:,19))&
                                              /(c_tomography(:,:,16)+c_tomography(:,:,19)+1.0e-20))
        write(IMAIN,*) ' max/min Gs :' ,maxval((c_tomography(:,:,17)+c_tomography(:,:,17))&
                                              /(c_tomography(:,:,16)+c_tomography(:,:,19)+1.0e-20)),&
                                        minval((c_tomography(:,:,17)+c_tomography(:,:,17))&
                                              /(c_tomography(:,:,16)+c_tomography(:,:,19)+1.0e-20))
...
!
  ...
  if (SUPPRESS_UTM_PROJECTION) then
    ...
    ! determine spacing and cell for linear interpolation
    spac_x = (lon_xmesh - ORIG_X(imat)) / SPACING_X(imat)
    spac_y = (lat_ymesh - ORIG_Y(imat)) / SPACING_Y(imat)
    !print*,"BinHe output 2",xmesh,ORIG_X(imat),spac_x,spac_y
  endif
  spac_z = (zmesh - ORIG_Z(imat)) / SPACING_Z(imat)
  ix = int(spac_x)
  iy = int(spac_y)
