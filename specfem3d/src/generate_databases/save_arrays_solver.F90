...
! outputs binary files for single mesh parameters (for example vp, vs, rho, ..)
  ...
  use shared_parameters, only: ACOUSTIC_SIMULATION, ELASTIC_SIMULATION, POROELASTIC_SIMULATION, &
    NPROC,ANISOTROPY,AZIMUTH_ANISOTROPY,TRANSVERSE_ANISOTROPY

  ! global indices
  use generate_databases_par, only: nspec => NSPEC_AB, ibool,ATTENUATION
  ...
  ! local parameters 
  ! BinHe put binary files together to avoid too may files
  real(kind=CUSTOM_REAL), dimension(:,:,:,:), allocatable :: vpv_tmp,vsv_tmp,rho_tmp,vph_tmp,vsh_tmp,gc_tmp,gs_tmp,v_tmp
  ...
  !--- x coordinate
  open(unit=IOUT,file=prname(1:len_trim(prname))//'xyz.bin',status='unknown',form='unformatted',iostat=ier)
  if (ier /= 0) stop 'error opening file xyz.bin'
  write(IOUT) xstore_unique,ystore_unique,zstore_unique
  close(IOUT)
  ...
  write(IOUT) ibool
  close(IOUT)
  allocate(v_tmp(NGLLX,NGLLY,NGLLZ,nspec))
  ...
  if(ANISOTROPY)then
    if(TRANSVERSE_ANISOTROPY)then
      allocate(vpv_tmp(NGLLX,NGLLY,NGLLZ,nspec),vsv_tmp(NGLLX,NGLLY,NGLLZ,nspec))
      allocate(vsh_tmp(NGLLX,NGLLY,NGLLZ,nspec),vph_tmp(NGLLX,NGLLY,NGLLZ,nspec))
      allocate(rho_tmp(NGLLX,NGLLY,NGLLZ,nspec))
      if (ier /= 0) call exit_MPI_without_rank('error allocating array 651')
      if (ier /= 0) call exit_MPI_without_rank('error allocating array')
      vpv_tmp = 0.0
      vsv_tmp = 0.0
      ...
      vsh_tmp = 0.0
      rho_tmp = 0.0
      where( rhostore /= 0._CUSTOM_REAL ) vph_tmp = sqrt(c11store / rhostore)
      where( rhostore /= 0._CUSTOM_REAL ) vpv_tmp  = sqrt(c33store / rhostore)
      where( rhostore /= 0._CUSTOM_REAL ) vsh_tmp = sqrt(c66store / rhostore)
      where( rhostore /= 0._CUSTOM_REAL ) vsv_tmp  = sqrt(0.5 * (c44store + c55store) / rhostore)
      rho_tmp=rhostore
      open(unit=IOUT,file=prname(1:len_trim(prname))//'vpvhvsvhrho.bin',status='unknown',form='unformatted',iostat=ier)
      if (ier /= 0) stop 'error opening file vpvhvsvhrho.bin'
      write(IOUT) vpv_tmp,vph_tmp,vsv_tmp,vsh_tmp,rhostore
      close(IOUT)
      vpv_tmp = 0.0
      vsv_tmp = 0.0
      rho_tmp = 0.0
      where( rho_vp /= 0._CUSTOM_REAL ) vpv_tmp = sqrt(0.5 * (c11store+c22store) / rhostore)
      where( rho_vp /= 0._CUSTOM_REAL ) rho_tmp = rhostore
      where( rho_vs /= 0._CUSTOM_REAL ) vsv_tmp = sqrt(0.5 * (c44store+c55store) / rhostore)
      open(unit=IOUT,file=prname(1:len_trim(prname))//'vpvsrho.bin',status='unknown',form='unformatted',iostat=ier)
      if (ier /= 0) stop 'error opening file vpvsrho.bin'
      write(IOUT) vpv_tmp,vsv_tmp,rho_tmp
      close(IOUT)
      deallocate(vpv_tmp,vph_tmp,vsv_tmp,vsh_tmp,rho_tmp)
    else if(AZIMUTH_ANISOTROPY)then
      allocate(vpv_tmp(NGLLX,NGLLY,NGLLZ,nspec),vsv_tmp(NGLLX,NGLLY,NGLLZ,nspec))
      allocate(vsh_tmp(NGLLX,NGLLY,NGLLZ,nspec),vph_tmp(NGLLX,NGLLY,NGLLZ,nspec))
      allocate(rho_tmp(NGLLX,NGLLY,NGLLZ,nspec))
      allocate(gs_tmp(NGLLX,NGLLY,NGLLZ,nspec),gc_tmp(NGLLX,NGLLY,NGLLZ,nspec))
      gc_tmp = 0.0
      gs_tmp = 0.0
      vsv_tmp = 0.0
      vsh_tmp = 0.0
      ...
      where( rhostore /= 0._CUSTOM_REAL ) v_tmp = 0.5* (c44store + c55store)
      where (v_tmp /= 0._CUSTOM_REAL)     gs_tmp = c45store / v_tmp
      where (v_tmp /= 0._CUSTOM_REAL)     gc_tmp = 0.5 * (c44store - c55store) / v_tmp
      where( rhostore /= 0._CUSTOM_REAL ) vsv_tmp = sqrt(c44store / rhostore)
      where( rhostore /= 0._CUSTOM_REAL ) vsh_tmp = sqrt(c55store / rhostore)

      open(unit=IOUT,file=prname(1:len_trim(prname))//'vs1vs2.bin',status='unknown',form='unformatted',iostat=ier)
      if (ier /= 0) stop 'error opening file vs1vs2.bin'
      write(IOUT) vsv_tmp,vsh_tmp
      close(IOUT)
      vpv_tmp = 0.0
      vsv_tmp = 0.0
      vph_tmp = 0.0
      ...
      where( rhostore /= 0._CUSTOM_REAL ) vsv_tmp = sqrt(0.5* (c44store+c55store) / rhostore)
      rho_tmp=rhostore
      open(unit=IOUT,file=prname(1:len_trim(prname))//'vpvhvsvhrhogcgs.bin',status='unknown',form='unformatted',iostat=ier)
      if (ier /= 0) stop 'error opening file vpvhvsvhrhogcgs.bin'
      write(IOUT) vpv_tmp,vph_tmp,vsv_tmp,vsh_tmp,rhostore,gc_tmp,gs_tmp
      close(IOUT)
      vpv_tmp = 0.0
      vsv_tmp = 0.0
      rho_tmp = 0.0
      ...
      write(IOUT) vpv_tmp,vsv_tmp,rho_tmp
      close(IOUT)
      deallocate(vpv_tmp,vph_tmp,vsv_tmp,vsh_tmp,rho_tmp,gc_tmp,gs_tmp)
    else
      open(unit=IOUT,file=prname(1:len_trim(prname))//'c21.bin',status='unknown',form='unformatted',iostat=ier)
      if (ier /= 0) stop 'error opening file c21.bin'
      write(IOUT) c11store,c12store,c13store,c14store,c15store,c16store, &
                           c22store,c23store,c24store,c25store,c26store, &
                                    c33store,c34store,c35store,c36store, &
                                             c44store,c45store,c46store, &
                                             c55store,c56store,c66store,rhostore
      close(IOUT)
      allocate(vpv_tmp(NGLLX,NGLLY,NGLLZ,nspec),vsv_tmp(NGLLX,NGLLY,NGLLZ,nspec))
      allocate(rho_tmp(NGLLX,NGLLY,NGLLZ,nspec))
      ...
      write(IOUT) vpv_tmp,vsv_tmp,rho_tmp
      close(IOUT)
      deallocate(vpv_tmp,vsv_tmp,rho_tmp)
    endif
  else
    allocate(vpv_tmp(NGLLX,NGLLY,NGLLZ,nspec),vsv_tmp(NGLLX,NGLLY,NGLLZ,nspec),rho_tmp(NGLLX,NGLLY,NGLLZ,nspec), stat=ier)
    ! vp (for checking the mesh and model)
    vpv_tmp = 0.0
    vsv_tmp = 0.0
    rho_tmp = 0.0
    where( rho_vp /= 0._CUSTOM_REAL ) vpv_tmp = (FOUR_THIRDS * mustore + kappastore) / rho_vp
    where( rho_vp /= 0._CUSTOM_REAL ) rho_tmp = rho_vp**2 / (FOUR_THIRDS * mustore + kappastore)
    where( rho_vs /= 0._CUSTOM_REAL ) vsv_tmp = mustore / rho_vs
    open(unit=IOUT,file=prname(1:len_trim(prname))//'vpvsrho.bin',status='unknown',form='unformatted',iostat=ier)
    if (ier /= 0) stop 'error opening file vpvsrho.bin'
    write(IOUT) vpv_tmp,vsv_tmp,rho_tmp
    close(IOUT)
  ...
  ! BinHe--2023-04-05 don't want to output too many files
  if (ATTENUATION) then
    ! shear attenuation

    if (myrank == 0) write(IMAIN,*) '     output : qmu and qkappa for attenuation'
    ! attenuation
    ...
    deallocate(v_tmp)
  endif ! BinHe--2023-04-05 don't want to output too many files
