...
!=====================================================================
  ...
  ! BinHe check anisotropy model parameter
  real(kind=CUSTOM_REAL) :: localmin,localmax,globmin,globmax
...
! DP: not sure if this here should check if (COUPLE_WITH_INJECTION_TECHNIQUE .or. MESH_A_CHUNK_OF_THE_EARTH) ..
  ...
  if(ANISOTROPY .and. IMODEL==IMODEL_TOMO)then
    ...
    if(myrank==0)write(IMAIN,*) ' Tomo Aniso parameter check....by BinHe '
     
     localmax=maxval((c44store-c55store)/(c44store+c55store+1.0e-20))
     localmin=minval((c44store-c55store)/(c44store+c55store+1.0e-20))
     call max_all_cr(localmax, globmax)
     call min_all_cr(localmin, globmin)
     if(myrank==0)write(IMAIN,*) ' min/max Gc :',globmin,globmax
     
     localmax=maxval((c45store+c45store)/(c44store+c55store+1.0e-20))
     localmin=minval((c45store+c45store)/(c44store+c55store+1.0e-20))
     call max_all_cr(localmax, globmax)
     call min_all_cr(localmin, globmin)
     if(myrank==0)write(IMAIN,*) ' min/max Gs :',globmin,globmax
     
     localmax=sqrt(maxval(c33store/rhostore))
     localmin=sqrt(minval(c33store/rhostore))
     call max_all_cr(localmax, globmax)
     call min_all_cr(localmin, globmin)
     if(myrank==0)write(IMAIN,*) ' min/max Vpv :',globmin,globmax
 
     localmax=sqrt(maxval(c11store/rhostore))
     localmin=sqrt(minval(c11store/rhostore))
     call max_all_cr(localmax, globmax)
     call min_all_cr(localmin, globmin)
     if(myrank==0)write(IMAIN,*) ' min/max Vph :',globmin,globmax
 
     localmax=sqrt(maxval((c44store+c55store)/rhostore*0.5))
     localmin=sqrt(minval((c44store+c55store)/rhostore*0.5))
     call max_all_cr(localmax, globmax)
     call min_all_cr(localmin, globmin)
     if(myrank==0)write(IMAIN,*) ' min/max Vsv :',globmin,globmax
     localmax=sqrt(maxval(c66store/rhostore))
     localmin=sqrt(minval(c66store/rhostore))
     call max_all_cr(localmax, globmax)
     call min_all_cr(localmin, globmin)
     if(myrank==0)write(IMAIN,*) ' min/max Vsh :',globmin,globmax
     if(myrank==0)write(IMAIN,*) ' ===================================== '
   endif
...
!
  ...
  use shared_parameters, only: COUPLE_WITH_INJECTION_TECHNIQUE,INJECTION_TECHNIQUE_TYPE,AZIMUTH_ANISOTROPY,TRANSVERSE_ANISOTROPY !BinHe added
...
! reads in material parameters from external binary files
  ...
  use shared_parameters, only: ANISOTROPY, AZIMUTH_ANISOTROPY, TRANSVERSE_ANISOTROPY !BinHe added
  ...
  case (IMODEL_GLL)
    ...
    if (ADIOS_FOR_MESH) then
      ...
      if(ANISOTROPY)call model_gll_aniso(myrank,nspec,LOCAL_PATH)
    endif
